...
import { generateAdminToken } from "../config/admin.token.js";
  ...
  try {
    ...
    if (!name || !email || !password || !role) {
      return res.status(401).json({
        success: false,
        error: "Please provide name, email, role & password field in body",
      });
    }
    ...
    const admin = await adminModal.create({ name, email, password, role });
